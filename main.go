...
func main() {
	...
	if srcCount != rowCount {
		log.Printf("找到的单据数和源数据不匹配: db(%d), src(%d)", len(srcDoc), rowCount)
	}
		...
		for _, datarow := range data.Data {
			...
			if strings.Contains(arRow["DocNo"], reqSODoc) {
				...
				if rm, er := strconv.ParseFloat(reqMoney, 64); err != nil {
					log.Printf("来源金额格式不正确：%s", er)
				} else {
					if rm != m {
						log.Printf("定金应收金额与源数据金额不一致：%s\t(%f),\t%s\t(%f)", arRow["DocNo"], m, reqSODoc, rm)
					}
				}
			}
		}
		if !found {
			log.Printf("没有找到应收单对应的来源订单号: %s", arRow["DocNo"])
		}
	}
	log.Printf("应收总金额为: %f", totalMoney)
